...
final class mysql {
	 ...
	 * @param $username 数据表用户名
	 * @param $password 数据表密码
	 * @param $database 数据表名字
	 * @param $charset  字符集 [默认为空，不设置字符集]
	 */
	 ...
	 * 选择数据库
	 * @param $database 数据表名字			
	 * @return boolean  false表示选择失败，true表示选择成功
	 */
	...
	/**
	 * 批量执行添加记录操作
	 * @param $datas 		要增加的数据，参数为二维数组。元素数组key为字段值，元素数组值为数据取值
	 * @param $table 		数据表
	 * @return boolean
	 */
	public function insert_batch($datas, $table,$replace = false) {
		if(!is_array( $datas ) || $table == '' || count($datas) == 0) {
			return false;
		}
		$querybatch=array();
		...
		$batchvalues=implode (',', $batchvalue);

		$cmd = $replace ? 'REPLACE INTO' : 'INSERT INTO';
		$sql = $cmd.' `'.$this->config['database'].'`.`'.$table.'`('.$field.') VALUES '.$batchvalues.'';
		$return = $this->execute($sql);
		return $return;
	}
	...
	public function start_transaction($autocommit=0) {
		return $this->execute("SET AUTOCOMMIT=$autocommit") && $this->execute("START TRANSACTION");
	}
